...
def recursive_print(obj_to_parse, header="", obj_type=None, idx=""):
    """
    ...
    if issubclass(type(obj_to_parse), (ctypes.Structure, ctypes.Union)):
        ...
        return '[' + ', '.join(recursive_print(item, header[:-1] + '[{}].'.format(idx), obj_to_parse._type_,idx) for idx, item in enumerate(obj_to_parse)) + ']'
...
class C2PyConverter(object):
    ...
    def _parse_enum(self, enum_match):
        ...
        for i, enum_data in enumerate(ENUM_TYPES_REGEX.finditer(enum_match.group("FIELDS"))):
            ...
            if enum_data.group("DEF") is None:
                ...
                for operand in re.finditer(r"\w*", trimmed_code_line):
    ...
    def _make_array(arrType, dimensions):
        ...
        for i in xrange(1, len(dimensions)):
            result = "({0} * dimensions[{1}])".format(result, int(i))
        dimensions = list(map(lambda x : int(x), dimensions)) # if char a[16/2][8] is used in c struct, it need to be changed as int
        return eval(result)
    ...
    def _parse_length(self, field_length):
        ...
        for match in re.finditer(r"\[.*?\]", field_length):
    ...
    def _get_exec_result(field_length):
        ...
        try:
            ...
            temp_field = field_length.replace(' ','')
            if temp_field == '(((((16))>((4)))?((16)):((4))))-(4)': # @todo It is workaround. Conditional operation is not considered in C2Py
                field_length = '16 - 4'
